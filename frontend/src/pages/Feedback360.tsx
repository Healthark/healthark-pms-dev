...
function MenteeFeedbackTab() {
  ...
  if (error) {
    return (
      <p className="rounded-lg bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 px-4 py-3 text-sm text-red-700 dark:text-red-300">
        {error}
      </p>
    );
  }
    ...
    <div className="space-y-4">
      <div>
        ...
        <select
          ...
          className="w-full sm:w-72 rounded-lg border border-border bg-surface px-3 py-2 text-sm text-text-main outline-none focus:border-brand"
        >
          {mentees.map((m) => (
    ...
    <div ref={wrapRef} className="relative">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-lg border border-border bg-surface pl-9 pr-9 py-2 text-sm text-text-main placeholder:text-text-muted outline-none focus:border-brand"
        />
        {value && !open ? (
          <button
            type="button"
      ...
      {open && (
        <ul
          role="listbox"
          className="absolute z-30 mt-1 w-full max-h-72 overflow-y-auto rounded-lg border border-border bg-surface shadow-lg"
        >
          {filtered.length === 0 ? (
...
function ReviewedBadge({ count }: { count: number }) {
  if (count > 0) {
    return (
      <span className="inline-flex items-center rounded-full bg-green-50 dark:bg-green-950/40 px-2 py-0.5 text-[10px] font-semibold text-green-700 dark:text-green-300 shrink-0">
        {count} review{count === 1 ? "" : "s"}
      </span>
    );
  }
  return (
    <span className="inline-flex items-center rounded-full bg-surface-hover px-2 py-0.5 text-[10px] font-semibold text-text-muted shrink-0">
      No reviews yet
    </span>
  );
}
